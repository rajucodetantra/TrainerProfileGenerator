...
if filtered.empty:
    st.warning("No trainer records found.")
    st.stop()


filtered["Display"] = (
    filtered["Emp ID"].astype(str)
    + " - "
...
try:
    ...
    st.error(f"Error fetching trainer details: {str(e)}")
    st.stop()
...
with left:
    ...
    for label, key in fields:
        ...
        value = str(trainer.get(key, "")).strip()
...
with right:
    ...
    st.subheader(
        trainer.get("Name", "Trainer Profile")
    )
    ...
    project_data = project_extractor.get_trainer_projects(
        lookup_id
    )


    if (
        project_data["ongoing"]
        or project_data["completed"]
    ):
...
if c1.button(
    ...
    st.success(
        "JSON Configuration Structure Checked"
    )
...
try:
    ...
    c2.error(
        f"Word Generation Failure: {str(e)}"
    )
...
try:
    ...
    result_pdf = service.generate_single(
        trainer.get("Emp ID"),
        output_format="PDF"
    )
...
try:
    ...
    result_both = service.generate_single(
        trainer.get("Emp ID"),
        output_format="Both"
    )
